...
import { Newspaper, Clock, ExternalLink, Loader2, AlertCircle, RefreshCw } from 'lucide-react';
...
const News = () => {
  ...
  const { data: newsArticles, isLoading, error, refetch, isFetching } = useCryptoNews();
  ...
  const filteredNews = newsArticles?.filter((article) => {
    ...
    return article.categories?.toUpperCase().includes(activeFilter) || 
      ...
      <section className="mb-6">
        <div className="flex items-center justify-between flex-wrap gap-4 mb-2">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-3xl md:text-4xl font-bold">{t('news.title')}</h1>
          </div>
          <Button 
            variant="outline" 
            size="sm" 
        ...
        <p className="text-xs text-muted-foreground mt-1">
          {language === 'ar' ? 'يتم التحديث تلقائياً كل 5 دقائق' : 'Auto-refreshes every 5 minutes'}
        </p>
      </section>
          ...
          <Card className="flat-card">
            <CardContent className="py-12 text-center">
              ...
              <p className="text-muted-foreground">
                {language === 'ar' ? 'لا توجد أخبار لهذا الفلتر' : 'No news found for this filter'}
              </p>
            </CardContent>
          </Card>
        )}
